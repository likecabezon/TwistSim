...
int main() {
    ...
    if(use_proximity_effect){
        ...
        dist_x = point_array[n_points].x - point_array[0].x;
        dist_y = point_array[n_points].y - point_array[0].y;
        dist_z = 0;
        dist_z_ref = -1 * height;

        dl_x = 0;
        dl_y = 0;
        dl_z = -1 * point_array[n_points].z;
        dl_z_ref = dl_z * -1;
        ...
        for(i = 1; i<(n_points); i++){
            ...
            dist_x = point_array[n_points].x -0.5*(point_array[i-1].x + point_array[i].x);
            dist_y = point_array[n_points].y -0.5*(point_array[i-1].x + point_array[i].x);
            dist_z = 0.5*(height - (point_array[i-1].z + point_array[i].z));
            dist_z_ref = -0.5*(height + point_array[i-1].z + point_array[i].z);

            dl_x = point_array[n_points].x;
            dl_y = point_array[n_points].y;
            dl_z = point_array[n_points].z;
            dl_z_ref = dl_z * -1;
        ...
        //añadir interaccion de [n_points] (shorted element)
        dist_x = point_array[0].x - point_array[n_points].x;
        dist_y = point_array[0].y - point_array[n_points].y;
        dist_z = 0;
        dist_z_ref = -1 * height;
        ...
        for(i=1; i<n_points;i++){
            ...
            magnetic_interactions[n_points][i][3] = MU_0*(magnetic_interactions[n_points][i][3] - (dot_prod_ref*point_array[n_points].x)/pow(point_array[n_points].z,2))/(pow(magnetic_interactions[n_points][i][7],3)*4*M_PI);
            magnetic_interactions[n_points][i][4] = MU_0*(magnetic_interactions[n_points][i][4] - (dot_prod_ref*point_array[n_points].y)/pow(point_array[n_points].z,2))/(pow(magnetic_interactions[n_points][i][7],3)*4*M_PI);
            magnetic_interactions[n_points][i][5] = 0;
        }
